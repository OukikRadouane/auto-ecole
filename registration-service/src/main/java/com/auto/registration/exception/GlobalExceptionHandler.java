...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleInvalidDocument(InvalidDocumentException ex) {
        log.error("Document invalide : {}", ex.getMessage());
    ...
    public ResponseEntity<ErrorResponse> handleValidationExceptions(MethodArgumentNotValidException ex) {
        ...
        return ResponseEntity.badRequest()
                .body(new ErrorResponse(HttpStatus.BAD_REQUEST.value(), "Validation échouée", errors, LocalDateTime.now()));
    }

    @ExceptionHandler(SecurityException.class)
    public ResponseEntity<ErrorResponse> handleSecurityException(SecurityException ex) {
        log.warn("Accès refusé / Violation de sécurité : {}", ex.getMessage());
    ...
    public ResponseEntity<ErrorResponse> handleGenericException(Exception ex) {
        ...
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(new ErrorResponse(HttpStatus.INTERNAL_SERVER_ERROR.value(),
                        "Une erreur inattendue s'est produite", LocalDateTime.now()));
    }
